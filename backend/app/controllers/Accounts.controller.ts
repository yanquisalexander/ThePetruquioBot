...
import TwitchAuthenticator, { TWITCH_SCOPES } from '../modules/TwitchAuthenticator.module'
...
class AccountsController {
  static async getToken(req: Request, res: Response) {
    try {
      ...
      const notificateNewLogin = new Notification({
        user,
  ...
  static async currentSession(req: Request, res: Response) {
    ...
    if (session.impersonatedUserId) {
      ...
      user.avatar = impersonatedUser.avatar!
  ...
  static async linkExternalAccount(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    ...
    // @ts-expect-error we are modifying the function
    ...
    switch (provider) {
      case ExternalAccountProvider.SPOTIFY:
        ...
        // @ts-expect-error showDialog no es un parámetro válido
      ...
      case ExternalAccountProvider.DISCORD:
        ...
        // @ts-expect-error prompt no es un parámetro válido
  ...
  static async linkExternalAccountCallback(req: Request, res: Response): Promise<Response | void> {
    try {
      ...
      switch (provider) {
        case ExternalAccountProvider.SPOTIFY:
          await AccountsController.handleLinkAccount(userAccount, ExternalAccountProvider.SPOTIFY, 'spotify', req, res)
          break
        case ExternalAccountProvider.DISCORD:
          await AccountsController.handleLinkAccount(userAccount, ExternalAccountProvider.DISCORD, 'discord', req, res)
          break
        case ExternalAccountProvider.PATREON:
          await AccountsController.handleLinkAccount(userAccount, ExternalAccountProvider.PATREON, 'patreon', req, res)
          break
        default:
          return res.status(400).json({ error: 'Invalid provider' })
      }
