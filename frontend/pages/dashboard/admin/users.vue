<template>
  <div class="mx-auto md:px-4 mb-16">
    <UAlert color="violet" icon="i-mdi-wrench-outline" variant="subtle" title="¡Cudado con lo que haces!">
    ...
    <div class="flex py-3.5 w-full">
      <UInput v-model="search" :placeholder="$t('users.search')" size="lg"
        leading-icon="i-heroicons-magnifying-glass-20-solid" />
      <div class="flex-grow" />
    </div>
    <div v-if="users" class="mt-2 bg-white rounded-md pt-4 px-4 w-full">
      <UTable :rows="displayedUsers" :columns="[
        ...
        <template #email-data="{ row }">
          <div class="flex items-center space-x-2">
            ...
            <UButton size="xs" variant="soft" color="green" @click="toggleEmailVisibility(row)">
        ...
        <template #actions-data="{ row }">
          <div class="flex space-x-2">
            <UTooltip text="Ver detalles" placement="top">
              <UButton size="xs" variant="soft" color="primary" :to="`/dashboard/users/${row.twitchId}`"
                icon="i-mdi-eye-outline">
                Ver detalles
              </UButton>
            </UTooltip>
            <UTooltip text="Eliminar usuario" placement="top">
              <UButton size="xs" variant="soft" color="red" icon="i-mdi-delete-outline" @click="showDeleteDialog(row)">
                Eliminar
              </UButton>
            </UTooltip>
            <UTooltip v-if="!row.admin" text="Impersonar usuario" placement="top">
              <UButton v-if="!row.admin" size="xs" variant="soft" color="violet" icon="i-mdi-account-switch-outline"
                @click="showImpersonateDialog(row)">
                Impersonar
              </UButton>
            </UTooltip>
            <UTooltip text="Actualizar Token" placement="top">
              <UButton size="xs" variant="soft" color="orange" icon="i-mdi-refresh" @click="refreshToken(row)">
                Actualizar Token
              </UButton>
            </UTooltip>
          </div>
      ...
      <div class="flex items-center justify-center mt-4">
        <UPagination v-model="currentUsersPage" :total="filteredUsers.length" />
      </div>
    </div>
      ...
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton variant="text" color="gray" @click="deleteDialog = false">
              {{ $t('dashboard.admin.cancel') }}
            </UButton>
            <UButton color="red" @click="deleteUser(selectedUser.twitchId)">
    ...
    <UModal v-model="impersonateDialog">
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton variant="text" color="gray" @click="closeImpersonateDialog">
              {{ $t('dashboard.admin.cancel') }}
            </UButton>
            <UButton color="primary" @click="impersonateUser(selectedUser.twitchId)">
