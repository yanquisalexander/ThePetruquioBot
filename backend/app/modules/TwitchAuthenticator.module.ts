...
import { RefreshingAuthProvider, AppTokenAuthProvider, type TokenInfoData, type AccessToken, exchangeCode } from '@twurple/auth'
...
class TwitchAuthenticator {
  ...
  public static async initialize (): Promise<void> {
    ...
    this.RefreshingAuthProvider = new RefreshingAuthProvider({
      ...
      // @ts-expect-error onRefresh is not typed
      onRefresh: async (userId: string, token: AccessToken) => {
        ...
        const botAccount = await User.findByTwitchId(parseInt(Configuration.TWITCH_USER_ID))
      ...
      onRefreshFailure: (userId: string, error: Error) => {
