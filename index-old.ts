...
const processMessage = async ( channel: string, userstate: ChatUserstate, username: string, message: string ) => {
    const isModerator = userstate.mod || Boolean(userstate.badges?.broadcaster) || userstate.username === 'alexitoo_uy' // Bot owner;
    const isBroadcaster = isModerator && userstate.badges?.broadcaster;
    ...
    if (isCommand) {
        ...
        await handleCommand({ channel, context: userstate, username, message, toUser: args[1], isModerator, settings: Settings, channelData });
    }

    if (Settings.enable_detoxify && !isModerator) {
        await handleDetoxify(channel, userstate, message, Settings);
    }
};



const onMessageHandler = async (channel: string, userstate: ChatUserstate, message: string, self: boolean) => {
    ...
    if (userstate.username === Bot.getUsername() || self || instanceId !== nextInstance) {
        // Ignorar mensajes si no es la instancia que debe procesarlos
        return;
    }

    processMessage(channel, userstate, userstate.username, message);
};

const onChatClearedHandler = (channel: string) => {
    console.log(chalk.bgWhite.magenta.bold(`Chat has been cleared in ${channel}`));
    //Bot.say(channel, getRandomOnClearChat());
};

const onWhisperHandler = async (from: string, userstate: ChatUserstate, message: string, self: boolean) => {
    ...
    await HelixClient.whispers.sendWhisper(process.env.TWITCH_USER_ID, toUser.id, `Hello ${userstate['display-name']}, I'm a bot created by Alexitoo_UY!`);
    setTimeout(() => {
...
const onNoticeHandler = async (channel: string, msgid: string, message: string) => {
...
const onRaidedHandler = async (channel: string, username: string, viewers: number) => {
...
setTimeout(async () => {
    ...
    Bot.on('join', (channel: string, username: string, self: boolean) => {
        if (self) {
    ...
    async function getNextInstance() {
        try {
            ...
            const instanceIds = allInstances.filter((_: any, index: number) => index % 2 === 0);
            const instanceScores = allInstances.filter((_: any, index: number) => index % 2 !== 0).map(Number);
