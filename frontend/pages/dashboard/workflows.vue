<template>
  <div class="mx-auto md:px-4 mb-16">
    <UAlert color="orange" icon="i-mdi-flask" variant="soft" title="¡Atención!"
    ...
    <div class="flex w-full my-4 space-x-2">
      <UInput v-model="search" placeholder="Buscar workflow" size="lg"
        leading-icon="i-heroicons-magnifying-glass-20-solid" />
      <div class="flex-grow" />
      <UButton color="gray" variant="ghost" icon leading-icon="i-lucide-file-code" @click="fetchLogs">
        Ver logs
      </UButton>
      <UButton color="primary" :disabled="loading" :loading="loading" icon leading-icon="i-mdi-plus"
        @click="showAddDialog">
        Añadir workflow
      </UButton>
    </div>
    ...
    <UTable :rows="filteredRows" :columns="headers" sort-asc-icon="i-heroicons-arrow-up-20-solid"
      ...
      <template #actions-data="{ row }">
        <div class="flex space-x-2">
          <UTooltip text="Editar workflow" placement="bottom">
            <UButton color="primary" variant="soft" icon leading-icon="mdi-pencil" @click="showEditDialog(row)">
              Editar
            </UButton>
          </UTooltip>
          <UTooltip text="Eliminar workflow" placement="bottom">
            <UButton color="red" variant="soft" icon leading-icon="mdi-trash-can" @click="showDeleteDialog(row)">
              Eliminar
            </UButton>
          </UTooltip>
        </div>
      ...
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="primary" @click="deleteDialog = false">
              Cancelar
            </UButton>
            <UButton color="red" @click="deleteWorkflow(selectedWorkflow.id)">
              Eliminar
            </UButton>
          </div>
        </template>
    ...
    <UModal v-model="addDialog"
      ...
      <UCard>
        ...
        <USelect v-model="newWorkflow.event_type" :options="workflowData.event_types" size="lg"
          placeholder="Selecciona un evento" icon="i-heroicons-sparkles-20-solid" class="my-2" />
        <div v-if="newWorkflow.event_type === 'ON_WEBHOOK'" class="flex items-center space-x-2">
          <UInput v-model="webhookUrl" readonly label="URL de webhook" disabled size="lg" class="flex-1" />
          <UTooltip text="Copiar URL de webhook" placement="bottom">
            <UButton color="primary" icon leading-icon="i-mdi-content-copy" @click="copyWebhookUrl">
              Copiar
            </UButton>
          </UTooltip>
        </div>
        ...
        <UAlert v-if="newWorkflow.event_type === 'ON_WEBHOOK'" color="blue" icon="i-mdi-flask" title="¡Atención!"
          class="my-4" variant="subtle">
          <template #description>
            <span class="font-jost font-normal">
              ...
              <br>Puedes acceder a los datos mediante la variable <UKbd class="bg-gray-100 cursor-pointer text-black"
                @click="newWorkflow.script = newWorkflow.script + 'data'">data</UKbd>.
            </span>
          </template>
        </UAlert>

        <UAlert v-if="newWorkflow.event_type === 'ON_WEBHOOK'" color="purple" icon="i-mdi-swap-vertical"
          title="Para tener en cuenta" class="my-4" variant="subtle">
          <template #description>
            <span class="font-jost font-normal">
        ...
        <vue-monaco-editor :value="newWorkflow.script" language="javascript" theme="vs-dark" :options="{
          minimap: {
            enabled: false,
          },
        }" height="350px" width="100%" @update:value="newWorkflow.script = $event" />
        <template #footer>
          <div class="flex justify-end space-x-2">
            <UButton color="gray" variant="ghost" @click="addDialog = false">
              Cancelar
            </UButton>
            <UButton color="primary" @click="addWorkflow">
              Añadir
            </UButton>
          </div>
        </template>
    ...
    <UModal v-model="editDialog"
      ...
      <UCard>
        ...
        <vue-monaco-editor :value="selectedWorkflow.script" language="javascript" theme="vs-dark" :options="{
          minimap: {
            enabled: false,
          },
        }" height="350px" width="100%" @update:value="selectedWorkflow.script = $event" />
        <template #footer>
          <div class="flex justify-end space-x-2">
            <UButton color="gray" variant="ghost" @click="editDialog = false">
              Cancelar
            </UButton>
            <UButton color="primary" @click="editWorkflow">
              Editar
            </UButton>
          </div>
        </template>
    ...
    <UModal v-model="experimentalAndAdvancedWarning"
      ...
      <UCard>
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="primary" @click="closeWarning">
              Entendido
            </UButton>
          </div>
        </template>
    ...
    <UModal v-model="logsDialog" prevent-close
      :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800', width: 'sm:max-w-screen-sm md:max-w-screen-md' }">
      <UCard>
        <template #header>
          <div class="text-h5">
        ...
        <div class="h-5/6 overflow-auto">
          <UTable :rows="displayedLogs" :columns="[
            ...
            <template #success-data="{ row }">
              <span v-if="row.success" class="text-green-500">Sí</span>
            ...
            <template #actions-data="{ row }">
              <div class="flex space-x-2">
                <UTooltip text="Ver registro" placement="bottom">
                  <UButton color="primary" variant="soft" icon leading-icon="mdi-file-document-edit"
                    @click="showLog(row)">
                    Ver
                  </UButton>
                </UTooltip>
                <UTooltip text="Eliminar registro" placement="bottom">
                  <UButton color="red" variant="soft" icon leading-icon="mdi-trash-can" @click="deleteLog(row)">
                    Eliminar
                  </UButton>
                </UTooltip>
              </div>
          ...
          <div class="flex justify-end px-3 py-3.5 border-t border-gray-200 dark:border-gray-700">
            <UPagination v-model="logsCurrentPage" :total="logs?.length" :page-count="6" />
          </div>
        </div>
        <template #footer>
          <div class="flex justify-end">
            <UButton color="primary" @click="logsDialog = false">
              Cerrar
            </UButton>
          </div>
        </template>
    ...
    <UModal v-model="singleLogDialog"
      ...
      {{ singleLog.executionLog }}
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="primary" @click="singleLogDialog = false">
              Cerrar
            </UButton>
          </div>
        </template>
...
const webhookUrl = computed(() => {
  return `https://api-petruquio.abstudios.tech/v2/webhooks/${workflowData.value?.user_api_token}`;
});
