...
import type ChatUser from '../utils/chat-user'
...
class CommandExecutor {
  ...
  async loadCommands(): Promise<Command[]> {
    ...
    for (const command of systemCommandsArray) {
  ...
  public async executeCommand(user: ChatUser, commandName: string, args: string[], channelData: Channel, bot: Bot): Promise<string | void> {
    ...
    if (systemCommand) {
      ...
      if (!command) {
        try {
          const channelCommands = await Command.getChannelCommands(channelData)
          if (channelCommands) {
            for (const channelCommand of channelCommands) {
          ...
          console.log(chalk.red('[COMMAND EXECUTOR]'), chalk.white(`Error while getting channel commands: ${(error as Error).message}`))
        }
      }
