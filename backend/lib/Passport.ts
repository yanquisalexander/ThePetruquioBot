...
class Passport {
  ...
  private static readonly passport = passport
  private static readonly jwtOptions = {
    ...
    passport.use('jwt', new JwtStrategy(this.jwtOptions, async (req, payload, done) => {
      ...
      try {
        ...
        const userData = {
          ...
          is_patron: isPatron,
          current_twitch_scopes: TwitchAuthenticator.RefreshingAuthProvider.hasUser(user.twitchId) ? TwitchAuthenticator.RefreshingAuthProvider.getCurrentScopesForUser(user.twitchId) : []
        }
