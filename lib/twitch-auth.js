...
await authProvider.addUserForToken(await readToken(twitchUserId), ['chat'])


// Función para leer el token de un usuario desde la base de datos
async function readToken(userId) {
  console.log(`Leyendo token para el usuario ${userId}`);
  try {
...
export async function getAccessToken(userId) {
  console.log(`Obteniendo token de acceso para el usuario ${userId}`);
  tokenData = await readToken(userId);
...
export const initializeAuth = async () => {
  ...
  tokenData = await readToken(twitchUserId);
